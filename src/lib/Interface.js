...
export default class Interface {
  ...
  buildStyle() {
    return el('style',
      ...
      '#cconsent-bar { background-color:' + window.CookieKonsent.config.theme.barColor + '; color:' + window.CookieKonsent.config.theme.barTextColor + '; padding:15px; text-align:right; font-family:sans-serif; font-size:14px; line-height:18px; position:fixed; bottom:0; left:0; width:100%; z-index:9998; transform: translateY(0); transition: transform .6s ease-in-out; transition-delay: .3s;}', 
      '#cconsent-bar.ccb--hidden {transform: translateY(100%); display:block;}', 
      '#cconsent-bar .ccb__wrapper { display:flex; flex-wrap:wrap; justify-content:space-between; max-width:1800px; margin:0 auto;}',
      '#cconsent-bar .ccb__left { align-self:center; text-align:left; margin: 15px 0;}',
      '#cconsent-bar .ccb__right { align-self:center; white-space: nowrap;}',
      '#cconsent-bar .ccb__right > div {display:inline-block; color:#FFF;}',
      '#cconsent-bar a { text-decoration:underline; color:' + window.CookieKonsent.config.theme.barTextColor + '; }',
      '#cconsent-bar button { line-height:normal; font-size:14px; border:none; padding:10px 10px; color:' + window.CookieKonsent.config.theme.barMainButtonTextColor + '; background-color:' + window.CookieKonsent.config.theme.barMainButtonColor + ';}',
      ...
      '#cconsent-modal .ccm__footer button:hover { background-color:' + Utilities.lightenDarkenColor(window.CookieKonsent.config.theme.modalMainButtonColor, -20) + '; }',
  ...
  buildBar() {
    return el('div#cconsent-bar.ccb--hidden',
        el(`div.ccb__wrapper`,
          el('div.ccb__left',
            el('div.cc-text', Language.getTranslation(window.CookieKonsent.config, window.CookieKonsent.config.language.current, 'barMainText'))
          ),
          el('div.ccb__right',
            el('div.ccb__button',
              el('a.ccb__edit', Language.getTranslation(window.CookieKonsent.config, window.CookieKonsent.config.language.current, 'barLinkSetting')),
              el('button.consent-give', Language.getTranslation(window.CookieKonsent.config, window.CookieKonsent.config.language.current, 'barBtnAcceptAll'))
            )
          )
        ),
      );
  ...
  buildModal() {
    ...
    var listCookies = function(category) {
      ...
      if(list.length) {
        ...
        for(let item in list) {
          listItems.push(el('li', Language.getTranslation(list[item], window.CookieKonsent.config.language.current, 'name')));
        }

        return [el('div.ccm__list', el('span.ccm__list__title', Language.getTranslation(window.CookieKonsent.config, window.CookieKonsent.config.language.current, 'modalAffectedSolutions')), el('ul', listItems))];
      }
    }
    ...
    function modalTabGroups() {
      ...
      for (let key in window.CookieKonsent.config.categories) {
        ...
        contentItems.push(el('dl.ccm__tabgroup' + '.' + key + ((window.CookieKonsent.config.categories[key].checked) ? '.checked-5jhk' : ''), {'data-category':key},
                            el('dt.ccm__tab-head', Language.getTranslation(window.CookieKonsent.config.categories[key], window.CookieKonsent.config.language.current, 'name'),
                            ...
                            el('dd.ccm__tab-content',
                              el('div.ccm__tab-content__left', 
                                ( ! window.CookieKonsent.config.categories[key].needed) && el('div.ccm__switch-component', el('div.status-off', Language.getTranslation(window.CookieKonsent.config, window.CookieKonsent.config.language.current, 'off')),
                                el('div.ccm__switch-group',
                                  el('label.ccm__switch',
                                    el('input.category-onoff', {type:'checkbox', 'data-category': key, 'checked': window.CookieKonsent.config.categories[key].checked}),
                                    el('span.ccm__switch__slider')
                                  )
                                ),
                                el('div.status-on', Language.getTranslation(window.CookieKonsent.config, window.CookieKonsent.config.language.current, 'on')))
                              ),
                              el('div.right',
                                el('h3', Language.getTranslation(window.CookieKonsent.config.categories[key], window.CookieKonsent.config.language.current, 'name')),
    ...
    return el('div#cconsent-modal',
      el('div.ccm__content',
        el('div.ccm__content__heading',
          el('h2', Language.getTranslation(window.CookieKonsent.config, window.CookieKonsent.config.language.current, 'modalMainTitle')),
          el('p',
            Language.getTranslation(window.CookieKonsent.config, window.CookieKonsent.config.language.current, 'modalMainText'),
            (window.CookieKonsent.config.modalMainTextMoreLink) ? el('a', { href: window.CookieKonsent.config.modalMainTextMoreLink, target: '_blank', rel: 'noopener noreferrer' }, Language.getTranslation(window.CookieKonsent.config, window.CookieKonsent.config.language.current, 'learnMore')) : null
          ),
        ...
        el('div.ccm__footer',
          el('button#ccm__footer__consent-modal-submit', Language.getTranslation(window.CookieKonsent.config, window.CookieKonsent.config.language.current, 'modalBtnSave')),
          el('button.consent-give', Language.getTranslation(window.CookieKonsent.config, window.CookieKonsent.config.language.current, 'modalBtnAcceptAll'))
        )
      )
    );
  }
  ...
  addEventListeners(elements) {
    ...
    this.elements['modal'].querySelector('.ccm__tabs').addEventListener('click', (event) => {
      ...
      if (event.target.classList.contains('category-onoff')) {
        window.CookieKonsent.config.categories[event.target.dataset.category].wanted =
        window.CookieKonsent.config.categories[event.target.dataset.category].checked = (event.target.checked === true) ? true : false;
