...
export default class Configuration {
  ...
  setConfiguration(configObject) {
    ...
    console.log(window.CookieKonsent.config, configObject, { ...window.CookieKonsent.config, ...configObject });
  ...
  cookieToConfig() {
    ...
    document.cookie.split(';').filter((item) => {
      ...
      if (item.indexOf('konsent') >= 0) {
        ...
        for (let key in cookieData.categories) {
          window.CookieKonsent.config.categories[key].checked = window.CookieKonsent.config.categories[key].wanted = (cookieData.categories[key].wanted === true) ? true : false;
        }
